...
Every constant used in the simulator's outcome model is documented here.
Marked `[sourced]` with a citation, or `[judgment]` as the author's own estimate.
...
| `insufficient_funds` share | 42% | `[sourced]` Razorpay payment failure analysis |
| `bank_downtime` share | 21% | `[sourced]` Razorpay downtime reports |
...
| UPI Autopay share | 50% | `[sourced]` NPCI UPI Autopay adoption data |
